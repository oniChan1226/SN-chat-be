...
export const addOfferedSkill = asyncHandler(async (req, res) => {
    ...
    if (duplicate) {
        throw new ApiError(400, "You already have this skill in your offered skills");
    }
...
export const addRequiredSkill = asyncHandler(async (req, res) => {
    ...
    if (duplicate) {
        throw new ApiError(400, "You already have this skill in your required skills");
    }
...
export const deleteOfferedSkill = asyncHandler(async (req, res) => {
    const { user } = req;
    ...
    // Check if skill exists in user's offered skills
    if (!skillProfile.offeredSkills.includes(skillId)) {
        throw new ApiError(404, "Skill not found in your offered skills");
    }

    // Remove from profile
    skillProfile.offeredSkills = skillProfile.offeredSkills.filter(
...
export const deleteRequiredSkill = asyncHandler(async (req, res) => {
    const { user } = req;
    const { skillId } = req.params;
    ...
    // Check if skill exists in user's required skills
    if (!skillProfile.requiredSkills.includes(skillId)) {
        throw new ApiError(404, "Skill not found in your required skills");
    }

    // Remove from profile
    skillProfile.requiredSkills = skillProfile.requiredSkills.filter(
        (id) => id.toString() !== skillId
    );
    await skillProfile.save();
